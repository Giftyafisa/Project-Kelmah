...
function ApplicationManagementPage() {
            ...
            if (activeJobs && activeJobs.length > 0) {
                try {
                    const allApplications = await Promise.all(
                        activeJobs.map(job => hirerService.getJobApplications(job.id, activeTab))
                    );
    ...
    const handleStatusUpdate = async () => {
        ...
        try {
            ...
            setSelectedApplication(updatedApplications.length > 0 ? updatedApplications[0] : null);
        } catch (err) {
    ...
    const handleMessage = async () => {
        ...
        try {
            const conv = await messagingService.createDirectConversation(selectedApplication.workerId);
        ...
        <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
            ...
            <Paper sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', height: isMobile ? 'auto' : 'calc(100vh - 200px)', overflow: 'hidden' }}>
                <Grid container>
                    ...
                    <Grid item xs={12} md={8} sx={{ p: 3, overflowY: 'auto', flex: '1 1 auto' }}>
                        {selectedApplication ? (
                            <>
                                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                                    <Avatar src={selectedApplication.workerAvatar} sx={{ width: 60, height: 60, mr: 2 }} />
                                    <div>
                                        <Typography variant="h5">{selectedApplication.workerName}</Typography>
                            ...
                            <Paper sx={{ p: 4, textAlign: 'center', background: 'transparent' }} elevation={0}>
            ...
            <Dialog open={showReviewDialog} onClose={() => setShowReviewDialog(false)} fullWidth maxWidth="sm">
                ...
                <DialogActions>
                    ...
                    <Button onClick={handleStatusUpdate} color={actionType === 'accepted' ? 'success' : 'error'} variant="contained" disabled={updating}>
                        {updating ? <CircularProgress size={24} /> : `Confirm ${actionType}`}
                    </Button>
                </DialogActions>
            </Dialog>
        </Container>
