...
const ConversationList = ({ 
      ...
      setLocalConversations(prevConvos => {
        const updated = prevConvos.map(convo => {
          ...
          return {
            ...
            unread: selectedConversationId === data.conversationId ? 0 : (convo.unread || 0) + 1
          };
        });
        // Reorder conversations by newest message
        return updated.sort((a, b) => new Date(b.latestMessage.timestamp) - new Date(a.latestMessage.timestamp));
      });
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      ...
      try {
        const results = await searchService.searchWorkers({ query: userSearchText, limit: 10 });
  ...
  const handleCreateConversation = async () => {
    try {
      ...
      if (newConversationType === 'direct') {
        ...
        convo = await messagingService.createGroupConversation(groupName, participantIds);
      }
      // Prepend and select
  ...
  return (
    <ConversationContainer elevation={3} role="region" aria-label="Conversations">
      ...
      >
        ...
        <MenuItem onClick={() => { setDateFilter('allDates'); handleFilterMenuClose(); }} selected={dateFilter === 'allDates'}>
          All Dates
        </MenuItem>
        <MenuItem onClick={() => { setDateFilter('today'); handleFilterMenuClose(); }} selected={dateFilter === 'today'}>
          Today
        </MenuItem>
        <MenuItem onClick={() => { setDateFilter('thisWeek'); handleFilterMenuClose(); }} selected={dateFilter === 'thisWeek'}>
          This Week
        </MenuItem>
        <MenuItem onClick={() => { setDateFilter('thisMonth'); handleFilterMenuClose(); }} selected={dateFilter === 'thisMonth'}>
          This Month
        </MenuItem>
      </Menu>
      ...
      >
        ...
        <DialogContent>
          ...
          <Autocomplete
            ...
            renderOption={(props, option) => (
              <li {...props} key={option.id}>
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  <Avatar src={option.avatar} alt={option.name} sx={{ width: 24, height: 24, mr: 1 }} />
                  <Typography variant="body2" sx={{ color: '#fff' }}>{option.name}</Typography>
                </Box>
              </li>
            )}
            ...
            value={selectedUsers}
            onChange={(event, value) => setSelectedUsers(Array.isArray(value) ? value : [value])}
            renderInput={(params) => (
              <TextField
                {...params}
