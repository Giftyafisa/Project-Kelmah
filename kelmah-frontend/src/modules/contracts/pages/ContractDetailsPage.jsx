...
const ContractDetailsPage = () => {
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 8 }}>
      ...
      <Grid container spacing={3}>
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                ...
                <Typography variant="body1" gutterBottom>{contract.clientName || 'N/A'}</Typography>
                
                <Typography variant="subtitle2" color="text.secondary">Worker</Typography>
                <Typography variant="body1" gutterBottom>{contract.workerName || 'N/A'}</Typography>
                
                <Typography variant="subtitle2" color="text.secondary">Contract Value</Typography>
                <Typography variant="body1" gutterBottom>${contract.value?.toFixed(2) || '0.00'}</Typography>
              </Grid>
              
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2" color="text.secondary">Start Date</Typography>
                <Typography variant="body1" gutterBottom>{formatDate(contract.startDate)}</Typography>
                
                <Typography variant="subtitle2" color="text.secondary">End Date</Typography>
                <Typography variant="body1" gutterBottom>{formatDate(contract.endDate)}</Typography>
                
                <Typography variant="subtitle2" color="text.secondary">Created On</Typography>
                <Typography variant="body1" gutterBottom>{formatDate(contract.createdAt)}</Typography>
              </Grid>
            </Grid>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            ...
            <Divider sx={{ mb: 2 }} />
            <Typography variant="body1">{contract.description || 'No detailed description provided.'}</Typography>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            ...
            {loading.milestones ? (
              ...
              <>
                <Stepper activeStep={milestones.filter(m => m.status === 'completed').length} alternativeLabel>
                    ...
                    <Card key={milestone.id} sx={{ mb: 2 }}>
                      <CardContent>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                          ...
                          <Typography variant="body2">
                            <strong>Amount:</strong> ${milestone.amount?.toFixed(2) || '0.00'}
                          </Typography>
                        </Box>
                        
                        {contract.status === 'active' && milestone.status === 'pending' && (
      ...
      <Dialog open={disputeDialogOpen} onClose={() => setDisputeDialogOpen(false)}>
        <DialogTitle>Raise a Dispute</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Please provide details about the issue you're experiencing with this contract.
          </DialogContentText>
          <TextField
            margin="dense"
